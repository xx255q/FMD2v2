...
function getpagenumber()
	...
	if HTTP.GET(rurl) then
		...
		if string.find(s, ' imgsrcs = new Array', 1, true) then
			s = GetBetween('(', ')', s)
		else
		...
		if HTTP.GET(script) then
			...
			if string.find(s, ',', 1, true) == nil then
				script = unp(HTTP.Document.ToString())
			elseif string.find(s, 'http', 1, true) == nil then
				script = unp(HTTP.Document.ToString())
	...
	if HTTP.GET(URL) then
		if string.find(URL:lower(), 'mangapicgallery.com/r/cspiclink', 1, true) then
			...
			for i = 0,dict.Count-1 do
				...
				if string.find(URL, t[1], 1, true) then ik = t[2]; break; end
			end
			local a = js.splitstr(ik, 'a')
			local s = require('fmd.imagepuzzle').Create(9, 9)
			for i = 1, #a do
